...
CREATE TABLE city (
	...
	countryID int,
	primary key (ID),
	constraint `cr_city_country_fk` foreign key(countryID)
...
create table user (
	...
	avatarID int,
	primary key (ID),
	constraint `cr_user_avatar_fk` foreign key (avatarID) references file(ID) on delete set null,
...
create table team (
	...
	avatarID int,
	primary key (ID),
	constraint `cr_team_avatar_fk` foreign key (avatarID) references file(ID) on delete set null,
